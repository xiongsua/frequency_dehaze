...
class Encoder(torch.nn.Module):
    def __init__(self, size):
        ...
        self.fc1 = torch.nn.Linear(int(128 * (size[2] // 16) * (size[3] // 16)), 100)
        self.fc2 = torch.nn.Linear(int(128 * (size[2] // 16) * (size[3] // 16)), 100)
...
class Decoder(torch.nn.Module):
    def __init__(self, size):
        super().__init__()
        self.linear0 = torch.nn.Linear(100, int(128 * (size[2] // 16) * (size[3] // 16)))
        self.size = size
    ...
    def forward(self, data):
        ...
        data = data.view(1, -1, self.size[2] // 16, self.size[3] // 16)
